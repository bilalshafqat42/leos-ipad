...
const WheybridgeUnit = () => {
  const { id } = useParams();
  const [paramId, setParamId] = useState();
  const [filteredObject, setFilteredObject] = useState();

  console.log("data", weybridgeDbs);
  ...
  useEffect(() => {
    if (paramId) {
      const filteredItem = weybridgeDbs.find((item) => item.id == paramId);
          ...
          <div className="col-5">
            <div className="unit-description-section">
              ...
              <table className="unit-description-section-block-list">
                <tr>
                  <th>Unit Number</th>
                  <td>WG-{filteredObject?.unitNo}</td>
                </tr>
                <tr>
                  <th>Unit Type </th>
